...
for(i=1; i<=nx; i++) {
    for(j=1; j<= ny; j++) {
	 ...
	 w1 = 1.0 / (8.0 * hx); // from guassian kernel
	 w2 = 1.0 / (4.0 * hx);
	 
	 df_dx = w1 * (f1[i+1][j+1] - f1[i-1][j+1]
		       + f1[i+1][j-1] - f1[i-1][j-1])
	       + w2 * (f1[i+1][j] - f1[i-1][j]); // derivative in the vertical direction
	 ...
	 df_dy = w3 * (f1[i+1][j+1] - f1[i+1][j-1]
		       + f1[i-1][j+1] - f1[i-1][j-1])
               + w4 * (f1[i][j+1] - f1[i][j-1]); // in horizontal direction
...
for(i=1; i<=nx; i++) {
    ...
    u[i][j] = (1 / det) * ((dxz[i][j] * dyy[i][j]) - (dxy[i][j]*dyz[i][j]));
    v[i][j] = (1 /det) * ((dxx[i][j]*dyz[i][j])-(dxz[i][j]*dxy[i][j]));
    c[i][j] = (trace*trace)/det;
...
inimage = fopen("/workspaces/Computer-vision/Assignment_2/ex02/pig1.pgm","rb"); /* opens the file in binary readmode and return a file pointer*/
if (inimage == NULL){
...
inimage = fopen("/workspaces/Computer-vision/Assignment_2/ex02/pig2.pgm","rb");
//gets (in2);
...
fgets(out1,sizeof(out1), stdin);

// Remove trailing newline character, if any
out1[strcspn(out1, "\n")] = '\0';
// Append .pgm if not already present
if (strstr(out1, ".pgm") == NULL) {
    strcat(out1, ".pgm");
}

printf("output image (flow magnitude):        ");
fgets(out2, sizeof(out2), stdin);
// Remove trailing newline character, if any
out2[strcspn(out2, "\n")] = '\0';
// Append .pgm if not already present
if (strstr(out2, ".pgm") == NULL) {
    strcat(out2, ".pgm");
}
